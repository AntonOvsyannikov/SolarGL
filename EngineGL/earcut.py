...
def earcut(data, holeIndices=None, dim=None):
    ...
    # if the shape is not too simple, we'll use z-order curve hash later; calculate p bbox
    if (len(data) > 80 * dim):
        minX = maxX = data[0]
        minY = maxY = data[1]
...
# create a circular doubly linked _list from p points in the specified winding order
...
# main ear slicing loop which triangulates a p (given as a linked _list)
def earcutLinked(ear, triangles, dim, minX, minY, size, _pass=None):
    if not ear:
        return

    # interlink p nodes in z-order
    ...
    while ear.prev != ear.next:
        ...
        # if we looped through the whole remaining p and can't find any more ears
        if ear == stop:
            ...
            if not _pass:
                ...
                # as a last resort, try splitting the remaining p into two
            elif _pass == 2:
...
# check whether a p node forms a valid ear with adjacent nodes
def isEar(ear):
    a = ear.prev
    b = ear
    c = ear.next
...
def isEarHashed(ear, minX, minY, size):
    ...
    while p and p.z <= maxZ:
        if p != ear.prev and p != ear.next and pointInTriangle(a.x, a.y, b.x, b.y, c.x, c.y, p.x, p.y) and area(p.prev,
                                                                                                                p,
                                                                                                                p.next) >= 0:
            return False
        p = p.nextZ
...
# go through all p nodes and cure small local self-intersections
...
# try splitting p into two and triangulate them independently
def splitEarcut(start, triangles, dim, minX, minY, size):
    # look for a valid diagonal that divides the p into two
    do = True
    a = start
    ...
    while do or a != start:
        ...
        while b != a.prev:
            if a.i != b.i and isValidDiagonal(a, b):
                # split the p in two by the diagonal
                c = splitPolygon(a, b)
...
# link every hole into the outer loop, producing a single-ring p without holes
...
# David Eberly's algorithm for finding a bridge between hole and outer p
...
# find the leftmost node of a p ring
...
# check if a diagonal between two p nodes is valid (lies in p interior)
...
# check if a p diagonal intersects any p segments
...
# check if a p diagonal is locally inside the p
...
# check if the middle point of a p diagonal is inside the p
...
# link two p vertices with a bridge; if the vertices belong to the same ring, it splits p into two;
...
class Node(object):
    def __init__(self, i, x, y):
        ...
        # previous and next vertice nodes in a p ring
        self.prev = None
        self.next = None
...
# return a percentage difference between the p area and its triangulation area;
...
# turn a p in a multi-dimensional array form (e.g. as in GeoJSON) into a form Earcut accepts
def flatten(data):
    dim = len(data[0][0])
    result = {
        'vertices': [],
...
def triangulate(polygon, holes=None):
    """
    Триангулирует полигон с вырезами. Нормально работает только с 2d точками
    В 3d портятся нормали
    Args:
        polygon: список точек
        holes: список списков точек
    Returns:
        verts, faces
            verts: список вершин
            faces: список граней, каждая грань список индексов в списке вершин

    """
    data = flatten((polygon,) + (tuple(holes) if holes else ()))
    tinds = earcut(data['vertices'], data['holes'], data['dimensions'])
    trias = tuple(tuple(tinds[i + j] for j in [0, 1, 2]) for i in range(0, len(tinds), 3))

    return concat(polygon, *(holes or ())), trias
